...
class IR_Lead_List_Table extends WP_List_Table {
    ...
    function column_tag_text ( $item )
    {
        //Build row actions
        $actions = array(
            'edit'    => sprintf('<div style="clear:both;"></div><a href="?page=%s&action=%s&tag=%s">Edit</a>', esc_attr($_REQUEST['page']), 'edit_list',$item['tag_id']),
            'delete'  => sprintf('<a href="?page=%s&action=%s&tag=%s">Delete</a>',esc_attr($_REQUEST['page']),'delete_list',$item['tag_id'])
        );
        
        //Return the title contents
        return sprintf('%1$s<br/>%2$s',
            /*$1%s*/ sprintf('<a class="row-title" href="?page=%s&action=edit_list&tag=%s">%s</a>', esc_attr($_REQUEST['page']), $item['tag_id'], $item['tag_text']),
