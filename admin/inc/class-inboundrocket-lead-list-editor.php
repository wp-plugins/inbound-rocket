...
class IR_Lead_List_Editor {
	 ...
	 * Class constructor
	 */
	function __construct ( $list_id = FALSE ) 
	{
		if ( $list_id )
     ...
     * @return  object
     */
	function get_tag_details ( $list_id )
	{
		global $wpdb;

		$q = $wpdb->prepare("SELECT * FROM {$wpdb->ir_tags} WHERE tag_id = %d", $this->list_id);
     ...
     * @param 	string
     * @param 	string
     * @return  int 		list_id of last inserted tag
     */
	function add_list ( $tag_text, $tag_form_selectors, $tag_synced_lists )
	{
		global $wpdb;
		
		$tag_text = sanitize_title($tag_text);
     ...
     * @return  bool
     */
	function save_list ( $list_id, $tag_text, $tag_form_selectors, $tag_synced_lists )
	{
		global $wpdb;
		
		$tag_text = sanitize_title($tag_text);
		
		$tag_slug = $this->generate_slug($tag_text, $list_id);

		$this->push_contacts_to_lead_list($list_id, $tag_synced_lists);

		$q = $wpdb->prepare("
			UPDATE $wpdb->ir_tags 
			SET tag_text = %s, tag_slug = %s, tag_form_selectors = %s, tag_synced_lists = %s
			WHERE tag_id = %d", $tag_text, $tag_slug, $tag_form_selectors, $tag_synced_lists, $list_id);
		$result = $wpdb->query($q);
     ...
     * @return  string
     */
	function generate_slug ( $tag_text, $list_id = 0 )
	{
		global $wpdb;
		...
		while ( $slug_used )
		{
			...
			$q = $wpdb->prepare("SELECT tag_slug FROM {$wpdb->ir_tags} WHERE tag_slug = %s " . ( $list_id ? $wpdb->prepare(" AND tag_id != %d ", $list_id) : '' ), $tag_slug_modified);
     ...
     * @return  object
     */
	function get_contacts_in_tagged_list ( $list_id = 0 )
	{
		global $wpdb;
		$q = $wpdb->prepare("SELECT contact_hashkey, lead_email, lead_first_name, lead_last_name FROM {$wpdb->ir_tag_relationships} ltr, {$wpdb->ir_leads} ll WHERE ltr.contact_hashkey = ll.hashkey AND ltr.tag_id = %d AND ltr.tag_relationship_deleted = 0", $list_id);
	...
	function push_contacts_to_lead_list ( $list_id = 0, $tag_synced_lists = '' )
	{
		...
		if ( count($contacts) && $tag_synced_lists )
		{
			...
			if ( count($synced_lists) )
			{
				...
				{
					$power_up_slug = $synced_list['esp'] . '_connect';  // e.g inboundrocket_mailchimp_connect_wp
